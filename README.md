...
- requests Cloudflare Workers AI (`@cf/lykon/dreamshaper-8-lcm`) for mayonnaise-style editing (img2img + optional inpainting mask),
...
CF_IMAGE_MODEL=@cf/lykon/dreamshaper-8-lcm
...
- **`Image generation failed` / partial result with no augmented image**  
  Verify `CF_ACCOUNT_ID` and `CF_API_TOKEN`, then check access to Workers AI and model path in `CF_IMAGE_MODEL` (default `@cf/lykon/dreamshaper-8-lcm`).
